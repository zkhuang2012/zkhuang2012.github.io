...
生信学习里装软件是一件很头疼又富有成就感的事（至少对于我来说）。
...
## 软件安装（开始唠叨了）
**1st**: 最希望的是开发者都编译好了下载解压直接用那种，比如blast+等。

**2nd**: 源代码(source code)安装，解压后，一定要好好看README.txt或者INSTALL，虽然一般来说就是三部曲。
```
# configure中通过--prefix=/path-to-install/来指定安装路径
./configure --prefix=/path-to-install/
# make编译可以通过-j4来设置线程数，这里是4个线程，加速编译
make -j4
make install
## 有的程序没有configure直接make，那怎么指定安装路径呢？
## 答案是make install DESTDIR=/path-to-install/
```
但configure过程往往都是各种error或者not found，然后复制错误提示去网上各种搜，可能缺某个lib，然后下载lib source code安装，再指定路径，可能折腾一圈发现连错误在哪都找不到。
...
而conda一是依赖镜像地址，一旦地址失效，就只能离线安装；二是环境设置不好容易混乱（可能只是我处理不来吧）。
总之，仁者见仁，能用就行。（ps:毕竟不是生信专业的，不要太较真。)

接下来，我会以实验室集群的环境进行案例演示
...
# axel是多线程下载，但有的网站不支持多线程，那时候再用wget
# 下载软件
axel -a -o $HOME/download https://mirrors.e-ducation.cn/CRAN/src/base/R-3/R-3.6.3.tar.gz
...
## 或者下载module source code然后执行以下命令
...
export PYTHONPATH=\$LOCAL_PYTHON/lib/python3.6/site-packages:$PYTHONPATH
...
source $HOME/.bashrc
```
PS：python2和python3是相互不兼容的额，所以当你装了多个python，在切换版本时，一定要确认$PYTHONPATH是否也对应切换了。

### 案例三：Perl module安装
既然python可以利用系统python，perl当然也可以。
而且十分不建议自己安装perl，过程比较麻烦，还要主要安装perl版本与系统版本是一致的，不然有的软件安装默认调用系统perl，一旦模块对不同版本不兼容，就会各种报错，相当郁闷。
```
